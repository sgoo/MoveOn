...
import gov.nasa.jpf.aprop.SequenceOp;
import gov.nasa.jpf.aprop.SequenceProcessor;
...
public class SequenceAnalyzer extends ListenerAdapter {
  ...
  public static final String SEQUENCE = "gov.nasa.jpf.annotation.Sequence";
  public static final String SEQUENCE_EVENT = "gov.nasa.jpf.annotation.SequenceEvent";
  public static final String SEQUENCE_METHOD = "gov.nasa.jpf.annotation.SequenceMethod";
  public static final String SEQUENCE_OBJECT = "gov.nasa.jpf.annotation.SequenceObject";
  ...
  // that is compatible to Alex Moffat's LGPLed SEQUENCE diagram editor
  ...
  class DefaultProcessor extends SequenceProcessor {
    ...
    public void processSequence (String id, Trace<SequenceOp> trace) {
      level=0;
      fname = config.getString("sequence.out", (config.getTarget() + '-' + nSequences + ".seq"));
          ...
          if (vn != null) {
            ...
            if (v != null) {
              if (v instanceof ElementInfo) {
                objRefs[i] = ((ElementInfo)v).getObjectRef();
              } else {
  ...
  void processInvokeInstruction (ThreadInfo ti, InvokeInstruction insn) {
    if (insn.isCompleted(ti) && !ti.isInstructionSkipped()) {
      ...
      //--- a sequence scope
      AnnotationInfo ai = getAnnotation(mi, SEQUENCE);
      if (ai != null) {
        updateSequenceScope(ai,ti,mi,insn);
      }
      
      // a sequence event
      ai = getAnnotation(mi, SEQUENCE_METHOD);
      if (ai != null) {
    ...
    if (ai != null) {
      ...
      if (idx >= 0) {
        ...
        if (si.containsVar(vn)) {
          ...
          int r = frame.getLocalVariable(insn.getLocalVariableIndex()); // this is post exec
  ...
  void processPUTFIELD (ThreadInfo ti, PUTFIELD insn) {
    ...
    if (fi.isReference()) {
      AnnotationInfo ai = getAnnotation(fi, SEQUENCE_OBJECT);
      if (ai != null){
        String id = ai.getValueAsString("id");
